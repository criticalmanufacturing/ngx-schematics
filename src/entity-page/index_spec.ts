...
describe('Generate Entity Page', () => {
  ...
  const pageEntityTypePath = `projects/${
    libraryOptions.name
  }/src/lib/page-${strings.dasherize(entityPageOptions.name)}`;
  const pageEntityTypeDetailsViewPath = `projects/${
    libraryOptions.name
  }/src/lib/page-${strings.dasherize(
    entityPageOptions.name
  )}/page-${strings.dasherize(entityPageOptions.name)}-details-view`;
  ...
  it('should generate the html file with `cmf-core-business-controls-entityPage` component selector', async () => {
    ...
    const entityPageTemplateContent = tree.readContent(
      `${pageEntityTypePath}/page-${strings.dasherize(
        entityPageOptions.name
      )}.component.html`
    );
  ...
  it('should have the Component decorator with properties selector, providers, templateUrl, and viewProviders', async () => {
    ...
    expect(entityPageContent).toContain(
      `selector: '${strings.dasherize(
        entityPageOptions.project
      )}-page-${strings.dasherize(entityPageOptions.name)}'`
    );
  ...
  it('should implement EntityPageInterface and LevelsToLoad', async () => {
    ...
    expect(entityPageContent).toContain(
      ...
      }.BusinessObjects.${strings.classify(
        entityPageOptions.name
      )}>, LevelsToLoad`
    );
  ...
  describe('- Generate Details View', () => {
    ...
    it('should be declared and exported in NgModule', async () => {
      ...
      expect(entityPageDetailsViewContent).toContain(
        `declarations: [Page${strings.classify(
          entityPageOptions.name
        )}DetailsViewComponent]`
      );
      expect(entityPageDetailsViewContent).toContain(
        `exports: [Page${strings.classify(
          entityPageOptions.name
        )}DetailsViewComponent]`
      );
      expect(entityPageDetailsViewContent).toContain(
