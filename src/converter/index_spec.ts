...
describe('Generate Converter', () => {
  ...
  it('should create the converter file', async () => {
    ...
    expect(
      tree.getDir(`${converterOptions.path}/${converterOptions.name}-converter`)
        .subfiles
    ).toEqual(
      jasmine.arrayContaining([`${converterOptions.name}-converter.pipe.ts`])
    );
  });
  ...
  it('should have the Pipe and Converter decorators', async () => {
    ...
    expect(pipeContent).toMatch(
      /import \{\s*Converter\s*\} from 'cmf-core-dashboards';/gm
    );
