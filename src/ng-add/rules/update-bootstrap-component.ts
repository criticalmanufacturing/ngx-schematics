 ...
 */
async function getBootstrapComponentPath(
  source: SourceFile
  ...
  const relativePath = source
    ...
    .find((impDec) =>
      impDec.getNamedImports().some((imp) => imp.getName() === componentSymbol)
    )
...
function getMetadataProperty(
  ...
  return metadata
    ...
    .find(
      (prop) =>
        prop.asKind(SyntaxKind.PropertyAssignment)?.getName() === propertyName
    )
 ...
 */
export function updateBootstrapComponent() {
  return async (tree: Tree) => {
    const modulePath = await getAppModulePath(tree);
