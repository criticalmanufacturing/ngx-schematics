...
export async function getMainPath(host: Tree): Promise<string | undefined> {
  ...
  const appProject = Array.from(workspace.projects.values()).find(
...
export async function getAppModulePath(
  host: Tree
  ...
  return findBootstrapModulePath(
    ts.createSourceFile(mainPath, mainContent, ts.ScriptTarget.Latest)
  );
}
