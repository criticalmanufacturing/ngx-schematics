...
function getMetadataClassDeclaration(
  source: SourceFile
): ClassDeclaration | undefined {
  return source
    .getClasses()
    .find((classNode) =>
      classNode.getExtends()?.getText().endsWith('PackageMetadata')
    );
}
...
export function insertMetadata(
  ...
  const accessor = allAccessors.find(
    (accessor) => accessor.getName() === propertyIdentifier
  );
...
public override get ${propertyIdentifier}(): ${
  ...
  const returnStatement = accessor.getFirstDescendantByKind(
    SyntaxKind.ReturnStatement
  );
  const array = returnStatement?.getFirstChildByKind(
...
export function insertRoutesMetadata(
  ...
  const allAccessors = metadataClass.getGetAccessors();
  const routesAccessor = allAccessors.find(
    (accessor) => accessor.getName() === 'routes'
  );

  const returnStatement = routesAccessor?.getFirstDescendantByKind(
    SyntaxKind.ReturnStatement
  );
 ...
 * @returns
 */
export function setPackageInfoMetadata(
  source: SourceFile,
  options: Required<PackageInfo>
): void {
  const metadataClass = getMetadataClassDeclaration(source);
  ...
  const allAccessors = metadataClass.getGetAccessors();
  const packageInfoAccessor = allAccessors.find(
    (accessor) => accessor.getName() === 'packageInfo'
  );
  const toInsert = `\
  /**
   * Package Info
...
export function updateObjectArrayProperty(
  ...
  const exportsMatch = /webpackExports\s*:\s*\[([^\]]*?)(\s*\])/.exec(
    loaderText
  );

  if (exportsMatch) {
    const insertIndex =
      exportsMatch.index + exportsMatch[0].length - exportsMatch[2].length;
    const baseIndentation =
      loader.getIndentationText().length / loader.getIndentationLevel();
    const indentation =
      loader.getIndentationText() + ' '.repeat(baseIndentation * 2);
...
export function updatePackageInfo(
  ...
  // Lets add the new element to the array of the metadata getter
  const returnStatement = packageInfoAccessor.getFirstDescendantByKind(
    SyntaxKind.ReturnStatement
  );
  const objectExpression = returnStatement?.getFirstChildByKind(
