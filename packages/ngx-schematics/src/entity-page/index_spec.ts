...
describe('Generate Entity Page', () => {
  ...
  const pageEntityTypePath = `projects/${libraryOptions.name}/src/lib/page-${strings.dasherize(
    entityPageOptions.name
  )}`;
  const pageEntityTypeDetailsViewPath = `projects/${
    libraryOptions.name
  }/src/lib/page-${strings.dasherize(entityPageOptions.name)}/page-${strings.dasherize(
    entityPageOptions.name
  )}-details-view`;
  ...
  it('should create the entity page files', async () => {
    ...
    const files = getAllFilesFromDir(
      `projects/${libraryOptions.name}/src/lib/page-${dasherizedEntityName}`,
      tree
    );
  ...
  it('should generate the html file with `cmf-core-business-controls-entityPage` component selector', async () => {
    ...
    const templateRegExp = new RegExp(
      `<cmf-core-business-controls-entityPage\\s*` +
        ...
        `i18n-entityTypeName="@@${strings.dasherize(
          entityPageOptions.project
        )}/page-${strings.dasherize(entityPageOptions.name)}#ENTITY_TYPE"\\s*` +
        `entityTypeName="${strings.nameify(entityPageOptions.name)}">\\s*` +
        `<\/cmf-core-business-controls-entityPage>`,
      'gm'
    );

    const entityPageTemplateContent = tree.readContent(
      `${pageEntityTypePath}/page-${strings.dasherize(entityPageOptions.name)}.component.html`
    );
  ...
  it('should have the Component decorator with properties selector, providers, templateUrl, and viewProviders', async () => {
    ...
    const entityPageContent = tree.readContent(
      `${pageEntityTypePath}/page-${strings.dasherize(entityPageOptions.name)}.component.ts`
    );
    expect(entityPageContent).toMatch(/@Component\(/);
    expect(entityPageContent).toContain(
      `selector: '${strings.dasherize(entityPageOptions.project)}-page-${strings.dasherize(
        entityPageOptions.name
      )}'`
    );
    expect(entityPageContent).toContain(`providers: [EntityPageService]`);
    expect(entityPageContent).toContain(
      `templateUrl: './page-${strings.dasherize(entityPageOptions.name)}.component.html'`
    );
    expect(entityPageContent).toContain(
  ...
  it('should implement EntityPageInterface and LevelsToLoad', async () => {
    ...
    expect(entityPageContent).toContain(
      `implements EntityPageInterface<Cmf.${
        entityPageOptions.namespace
      }.BusinessObjects.${strings.classify(entityPageOptions.name)}>, LevelsToLoad`
    );
    expect(entityPageContent).toContain(`public epLevelsToLoad = 1;`);
    expect(entityPageContent).toContain(
      `public epEntity: Cmf.${entityPageOptions.namespace}.BusinessObjects.${strings.classify(
        entityPageOptions.name
      )};`
    );
  });
  ...
  it('should have the constructor receiving the ViewContainerRef and providing it to the super', async () => {
    ...
    expect(entityPageContent).toMatch(
  ...
  it('should import CommonModule and EntityPageModule in NgModule', async () => {
    ...
    expect(entityPageContent).toMatch(
      /imports: \[\s*((CommonModule|EntityPageModule)\s*,?\s*){2}\]/gm
    );
  });
  ...
  it('should be declared and exported in NgModule', async () => {
    ...
    expect(entityPageContent).toContain(
      `declarations: [Page${strings.classify(entityPageOptions.name)}Component]`
    );
    expect(entityPageContent).toContain(
      `exports: [Page${strings.classify(entityPageOptions.name)}Component]`
    );
    expect(entityPageContent).toContain(
      `export class Page${strings.classify(entityPageOptions.name)}Module { }`
    );
  });
  ...
  it('should get the routes for an entity in Routing Module', async () => {
    ...
    const routingModuleContent = tree.readContent(
      `${pageEntityTypePath}/page-${strings.dasherize(entityPageOptions.name)}-routing.module.ts`
    );
    const getRoutes = `const routes: Routes = EntityTypeMetadataService.getRoutes('${strings.classify(
      entityPageOptions.name
    )}', Page${strings.classify(entityPageOptions.name)}Component);`;
  ...
  it('should import Page Module and Router Module', async () => {
    ...
    const routingModulesImportsRegExp = new RegExp(
      `imports: \\[\\s*((RouterModule\\.forChild\\(routes\\)|Page${strings.classify(
        entityPageOptions.name
      )}Module)\\s*,?\\s*){2}\\]`,
      'gm'
    );
  ...
  it('should export the Routing Module', async () => {
    ...
    expect(routingModuleContent).toContain(
      `export class Page${strings.classify(entityPageOptions.name)}RoutingModule { }`
    );
  });
  ...
  describe('- Generate Details View', () => {
    it('should generate the html file with `cmf-core-business-controls-detailsView` component selector', async () => {
      ...
      const entityPageDetailsViewTemplateContent = tree.readContent(
        `${pageEntityTypeDetailsViewPath}/page-${strings.dasherize(
          entityPageOptions.name
        )}-details-view.component.html`
      );
      expect(entityPageDetailsViewTemplateContent).toMatch(
        /<cmf-core-business-controls-detailsView \*ngIf="epEntity">\s*<\/cmf-core-business-controls-detailsView>/gm
      );
    });
    ...
    it('should have the Component decorator with properties selector and templateUrl', async () => {
      ...
      expect(entityPageContent).toContain(
        `templateUrl: './page-${strings.dasherize(
          entityPageOptions.name
        )}-details-view.component.html'`
      );
    });
    ...
    it('should extend CustomizableComponent', async () => {
      ...
      const entityPageDetailsViewContent = tree.readContent(
        `${pageEntityTypeDetailsViewPath}/page-${strings.dasherize(
          entityPageOptions.name
        )}-details-view.component.ts`
      );
      expect(entityPageDetailsViewContent).toContain(
        `export class Page${strings.classify(
          entityPageOptions.name
        )}DetailsViewComponent extends CustomizableComponent`
      );
    });
    ...
    it('should have the constructor receiving the ViewContainerRef and EntityPageService', async () => {
      ...
      expect(entityPageDetailsViewContent).toMatch(
        /constructor\(viewContainerRef: ViewContainerRef, private entityPage: EntityPageService\)/gm
      );
    ...
    it('should have the epOnEntityLoaded method', async () => {
      ...
      const epOnEntityLoadedMethodRegExp = new RegExp(
        `public epOnEntityLoaded\\(\\) {\\s*` +
          `this\\.epEntity = this\\.entityPage\\.epEntity as Cmf\\.${
            entityPageOptions.namespace
          }\\.BusinessObjects\\.${strings.classify(entityPageOptions.name)};` +
          `\\s*}`,
        'gm'
      );
    ...
    it('should import CommonModule and DetailsViewModule in NgModule', async () => {
      ...
      expect(entityPageDetailsViewContent).toMatch(
        /imports: \[\s*((CommonModule|DetailsViewModule)\s*,?\s*){2}\]/gm
      );
    });
    ...
    it('should be declared and exported in NgModule', async () => {
      ...
      expect(entityPageDetailsViewContent).toContain(
        `declarations: [Page${strings.classify(entityPageOptions.name)}DetailsViewComponent]`
      );
      expect(entityPageDetailsViewContent).toContain(
        `exports: [Page${strings.classify(entityPageOptions.name)}DetailsViewComponent]`
      );
      expect(entityPageDetailsViewContent).toContain(
        `export class Page${strings.classify(entityPageOptions.name)}DetailsViewModule { }`
      );
    });
    ...
    it('should have the Details View route defined in Routing Module', async () => {
      ...
      const routingModuleContent = tree.readContent(
        `${pageEntityTypeDetailsViewPath}/page-${strings.dasherize(
          entityPageOptions.name
        )}-details-view-routing.module.ts`
      );
      const detailsViewRouteRegExp = new RegExp(
        `{\\s*path: '',\\s*component: Page${strings.classify(
          entityPageOptions.name
        )}DetailsViewComponent\\s*}`,
        'gm'
      );
